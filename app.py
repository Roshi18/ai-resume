...
st.set_page_config(
    page_title="Web-Based AI Interview Coach",
    page_icon="🚀",
    layout="wide"
)
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "🎯 AI INTERVIEW SIMULATOR" : self.render_ai_interview,
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            st.title("Web-Based AI Interview Coach")
            st.markdown("---")
    ...
    def render_ai_interview(self):
        st.title("🎯 AI Interview Simulator")

        # -----------------------------
        # Phase 1: Resume Upload & Question Generation
        # -----------------------------
        st.header("Phase 1: Resume Analysis & Question Generation")
                ...
                with st.spinner("Analyzing your resume..."):
                    ...
                    try:
                        response = requests.post(
                            "http://127.0.0.1:8000/analyze_resume/", files=files
                        )
        ...
        # -----------------------------
        st.header("Phase 2: Record Your Answers")
        ...
        def recognize_speech(file_path):
            ...
            try:
                ...
                return "Sorry, could not understand your speech. Try speaking louder or clearer."
        ...
        # Load NLP model and grammar tool once
        ...
        def evaluate_answer_local(question, answer):
            ...
            # Fluency: Average sentence length
            sentences = list(doc.sents)
            avg_sentence_length = sum(len(sent) for sent in sentences) / len(sentences) if sentences else 0
            fluency_score = min(10, max(0, (avg_sentence_length / 20) * 10))  # Scale to 0-10

            # Vocabulary: Unique words count
            unique_words = set(token.lemma_.lower() for token in doc if token.is_alpha)
            vocabulary_score = min(10, (len(unique_words) / 50) * 10)  # Scale to 0-10

            # Grammar: LanguageTool errors
            matches = tool.check(answer)
            grammar_errors = len(matches)
            grammar_score = max(0, 10 - grammar_errors)  # Deduct points for errors

            # Overall score
            overall_score = round((fluency_score + vocabulary_score + grammar_score) / 3, 2)
            ...
            if fluency_score >= 7:
                strengths.append("Good fluency with well-structured sentences.")
            else:
                weaknesses.append("Fluency could be improved.")
                suggestions.append("Try to use varied sentence structures and avoid long, complex sentences.")

            if vocabulary_score >= 7:
                strengths.append("Rich vocabulary with diverse word usage.")
            else:
                weaknesses.append("Limited vocabulary.")
                suggestions.append("Incorporate more varied and sophisticated words.")

            if grammar_score >= 7:
                strengths.append("Strong grammatical accuracy.")
            else:
                weaknesses.append("Several grammatical errors detected.")
                suggestions.append("Review common grammar rules and consider using grammar checking tools.")
        ...
        with col1:
            if st.button("🎙️ Start Recording"):
                with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
                    recording, samplerate = record_audio(duration=7)
                    wavio.write(tmp_file.name, recording, samplerate, sampwidth=2)
                    st.session_state.recording = tmp_file.name
                    st.audio(tmp_file.name, format="audio/wav")  # playback
                    st.success(f"Recording saved temporarily as {tmp_file.name}")

        with col2:
            if st.button("⏹️ Stop Recording"):
                if st.session_state.recording:
                    text = recognize_speech(st.session_state.recording)
                    st.subheader("🗣️ Your Answer (Transcribed):")
                    st.write(text)
                
                    st.subheader("⚡ Instant Feedback & Evaluation")
                    selected_question = questions[0] if questions else "Tell me about yourself"
                    feedback = evaluate_answer_local(selected_question, text)
            ...
            # Convert scores to percentage for progress bars
                    keyword_percent = min(int((sum(1 for k in ["education","skills","experience"] if k in text.lower())/3)*100),100)
                    semantic_percent = int((nlp(feedback['strengths'][0] if feedback['strengths'] else "").similarity(nlp(text))) * 100) if feedback['strengths'] else 50
                    grammar_percent = max(0, min(int((10 - len(tool.check(text))/2)*10), 100))

                    st.text("🔹 Relevance / Keywords")
                    st.progress(keyword_percent)
                    st.text("🔹 Semantic Understanding")
                    st.progress(semantic_percent)
                    st.text("🔹 Grammar & Fluency")
                    st.progress(grammar_percent)
        ...
        if questions:
            st.subheader("Questions for Reference:")
            st.write(questions)
